...
void loop() {
 ...
 SetSpeedRight = SetSpeedx + (0.57*SetSpeedy*2/3);
  ...
  if(ErrorRight > 0.8 * SetSpeedRight)
    ErrorRight = 0;
  OutputLeft = scale*((SetSpeedLeft) + (ErrorLeft * Kpl + (ErrorLeft - OldErrorLeft) * Kd + Ki * SumErrorLeft));
  OutputRight = scale*((SetSpeedRight) + (ErrorRight * Kpr + (ErrorRight - OldErrorRight) * Kd + Ki * SumErrorRight));
  cap(OutputLeft,300);
  cap(OutputRight,300);
  
  ST.motor(2,-OutputRight);
  ST.motor(1,OutputLeft);
